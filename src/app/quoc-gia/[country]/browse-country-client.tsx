...
export const BrowseCountryClient: React.FC<BrowseCountryClientProps> = ({
  country,
  initialSearchParams
}) => {
  ...
  const { countries: countriesData, genres: genresData } = useMetadata();
  ...
  const { data: moviesData, isLoading, error } = useMoviesByCountry(country, filters);
  
  const countryData = countriesData?.find(c => c.slug === country);
  const pageTitle = countryData?.name || 'Quốc gia';
  const pageSubtitle = `Phim xuất xứ từ ${pageTitle}`;
  ...
  const clearFilters = () => {
    ...
    setSelectedLang('');
    setCurrentPage(1);
    updateURL({ genre: '', year: '', lang: '', page: '1' });
  };
      ...
      <div className="min-h-screen">
            ...
            <div className="flex flex-col lg:flex-row gap-4 items-start lg:items-center justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                {selectedGenre && (
                  ...
                  >
                    Thể loại: {genresData?.find(g => g.slug === selectedGenre)?.name} ×
                  </Badge>
                )}
                {selectedYear && (
                  <Badge 
        ...
        <div className="container mx-auto px-4 pb-12">
          <MovieGrid
            ...
            emptyMessage={`Không tìm thấy phim từ ${pageTitle}`}
          />
        </div>
      </div>
    </Layout>
...
export default BrowseCountryClient;
